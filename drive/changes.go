...
func (self *Drive) ListChanges(args ListChangesArgs) error {
	...
	changeList, err := self.service.Changes.List(args.PageToken).IncludeItemsFromAllDrives(true).PageSize(args.MaxChanges).RestrictToMyDrive(true).Fields("newStartPageToken", "nextPageToken", "changes(fileId,removed,time,file(id,name,md5Checksum,mimeType,createdTime,modifiedTime))").Do()
	if err != nil {
